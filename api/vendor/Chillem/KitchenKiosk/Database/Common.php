...
/*
 * Primary database handler class
 *
 * @class Common
 *
 * @param \PDO $PDO
...
class Common {
    ...
    public function loadConfig(){
        ...
        try {
            $sth = $this->PDO->prepare("SELECT * FROM config");
            $sth->execute();
    ...
    public function getCredentialsByUsername($username){
        ...
        try {
            $sth = $this->PDO->prepare("SELECT mid, username, email, password FROM user WHERE username=:username AND enabled='1'");
    ...
    public function getCredentialsByEmail($email){
        ...
        try {
            $sth = $this->PDO->prepare("SELECT mid, username, password FROM user WHERE email=:email AND enabled='1'");
    ...
    public function getUserByUsername($username){
        ...
        try {
            $sth = $this->PDO->prepare("SELECT mid, name, username, email FROM user WHERE username=:username");
            $sth->bindValue(":username", $username);
            $sth->execute();
            $row = $sth->fetch(\PDO::FETCH_ASSOC);
            $sth->closeCursor();
    ...
    public function getUserByEmail($email){
        //load general data for a user
        try {
            $sth = $this->PDO->prepare("SELECT mid, name, username, email FROM user WHERE email=:email");
            $sth->bindValue(":email", $email);
            $sth->execute();
            $row = $sth->fetch(\PDO::FETCH_ASSOC);
            $sth->closeCursor();
    ...
    public function getUserById($mid){
        //load general data for a user
        try {
            $sth = $this->PDO->prepare("SELECT mid, name, username, email FROM user WHERE mid=:mid");
    ...
    public function insertUser($name, $username, $email, $password){
        //insert a new user
        try {
            $sth = $this->PDO->prepare("INSERT INTO user (name, username, email, password, created) VALUES(:name, :username, :email, :password, NOW())");
    ...
    public function updatePassword($username, $password, $email=false){
        ...
        try {
            if ( $email ){
                $sth = $this->PDO->prepare("UPDATE user SET password=:password WHERE email=:username");
            } else {
                $sth = $this->PDO->prepare("UPDATE user SET password=:password WHERE username=:username");
            }
            $sth->bindValue(":username", $username);
            $sth->bindValue(":password", $password);
            $sth->execute();
    ...
    public function toggleUserStatus($mid, $enabled){
        ...
        try {
            $sth = $this->PDO->prepare("UPDATE user SET enabled=:enabled WHERE mid=:mid");
    ...
    public function createSession($mid, $ip, $token, $time){
        ...
        try {
            ...
            $sth = $this->PDO->prepare("INSERT INTO session (mid, ip, expires, token) VALUES(:mid, :ip, ( NOW() + INTERVAL " . $time . " ), :token)");
            $sth->bindValue(":mid", $mid);
            $sth->bindValue(":ip", $ip);
    ...
    public function updateSession($token, $time){
        ...
        try {
            ...
            $sth = $this->PDO->prepare("UPDATE session SET expires=( NOW() + INTERVAL " . $time . " ) WHERE token=:token");
            $sth->bindValue(":token", $token);
            $sth->execute();
            $sth->closeCursor();
        } catch ( \PDOException $e ){
    ...
    public function validateToken($username, $token){
        ...
        try {
            $sth = $this->PDO->prepare("SELECT COUNT(s.*) AS count FROM session s JOIN user u USING (mid) WHERE u.username=:username AND s.token=:token AND s.expires>NOW()");
            $sth->bindValue(":username", $username);
            $sth->bindValue(":token", $token);
            $sth->execute();
            $row = $sth->fetch(\PDO::FETCH_ASSOC);
    ...
    public function purgeExpiredSession($mid=null){
        ...
        try {
            if ( $mid ){
                $sth = $this->PDO->prepare("DELETE s, c FROM session s JOIN cookies c USING (mid) WHERE s.mid=:mid AND s.expires<NOW()");
                $sth->bindValue(":mid", $mid);
            } else {
                $sth = $this->PDO->prepare("DELETE s, c FROM session s JOIN cookies c USING (mid) WHERE s.expires<NOW()");
            }
            $sth->execute();
            $sth->closeCursor();
        } catch ( \PDOException $e ){
    ...
    public function purgeSessionByUsername($username){
        ...
        try {
            $sth = $this->PDO->prepare(" DELETE s, c FROM session s JOIN cookies c USING (mid) JOIN user u USING (mid) WHERE u.username=:username");
            $sth->bindValue(":username", $username);
            $sth->execute();
            $sth->closeCursor();
        } catch ( \PDOException $e ){
    ...
    public function saveCookie($mid, $cookie){
        ...
        try {
            $sth = $this->PDO->prepare("INSERT INTO cookies (mid, cookie) VALUES(:mid, :cookie)");
            $sth->bindValue(":mid", $mid);
    ...
    public function validateCookie($username, $cookie){
        ...
        try {
            $sth = $this->PDO->prepare("SELECT COUNT(c.*) AS count, c.mid AS mid FROM cookie c JOIN user u USING (mid) WHERE u.username=:username AND c.cookie=:cookie");
            $sth->bindValue(":username", $username);
            $sth->bindValue(":cookie", $cookie);
            $sth->execute();
            $row = $sth->fetch(\PDO::FETCH_ASSOC);
            ...
            $sth = $this->PDO->prepare("DELETE c.* FROM cookie c JOIN user u USING (mid) WHERE u.username=:username AND c.cookie=:cookie");
            $sth->bindValue(":username", $username);
            $sth->bindValue(":cookie", $cookie);
            $sth->execute();
            $sth->closeCursor();
